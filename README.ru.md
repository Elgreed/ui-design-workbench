...
- Runtime-free перенос Android, Apple и Flutter UI с детерминированной geometry для поддержанного подмножества; нативные evidence нужны только перед заявлением о визуальном или pixel parity и никогда не запускаются автоматически.
...
- HTML — детерминированная runtime-free проекция исходников для поддержанного общего layout-подмножества, а не доказательство runtime- или pixel-parity.
- Перенос Android, Apple и Flutter использует детерминированную geometry для поддержанного общего layout-подмножества; без отдельного нативного evidence это структурная проекция, а не доказательство runtime- или pixel-parity.
