...
7. Build the editor chrome with [references/workbench-ui.md](references/workbench-ui.md). Provide All screens, Prototype, Single screen, and Compare. Keep Problems as an independent canvas layer. Choose one active version in the document bar for ordinary views; make Compare a dedicated view with explicit left/right version selectors and its own split/overlay controls. Keep Interact, Inspect, and Comment independent. Iterate in IR, never in generated HTML.
...
- Keep Screens, Properties, Review, Comments, File, locale, and right-panel visibility directly reachable from the narrow rail. File and locale are icon menus; they must remain accessible, mutually exclusive, localized, and must not duplicate a persistent document-bar menu.
