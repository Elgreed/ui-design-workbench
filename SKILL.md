...
Use `init` for an explicit first scan, `status` to inspect freshness, `sync` after known UI changes, `diff` for impact, and `context --screen <id-or-name> --budget <tokens>` for bounded screen work. `init --mock-data representative|exhaustive` opts into deterministic synthetic fixtures; later use `mock-data --set ...` and `scenarios validate`. The default is `none`. Representative mode creates one populated fixture plus only task-relevant critical states, never a universal state trio. The default derived state belongs in the operating-system user cache; use `init --project-cache` only when the user explicitly needs ignored project-local state for CI or a portable environment. See [references/cache-protocol.md](references/cache-protocol.md).
...
5. Create a separate `<review-dir>/ui-ir.json` using [references/ir-schema.md](references/ir-schema.md). Keep review artifacts outside the source repository unless explicitly requested. When mock data is enabled, store reusable deterministic patches in `scenarioFixtures` and sparse, screen-specific references in `screen.scenarios`; preserve them for unaffected screens during sync. Model every source-evidenced data collection as a `list`, `table`, `grid`, or `collection` with repeated item nodes. A populated scenario must replace its empty-state child with at least two visibly synthetic items; a summary string is not a populated collection. The cached starter IR is inventory, not a finished design.
...
8. Prefer `uidw workbench` for sync/render/check/open orchestration and `uidw check --level quick|full` for consolidated gates; use the lower-level commands documented in [references/validation.md](references/validation.md) and [references/quality-automation.md](references/quality-automation.md) when a custom matrix is required. Report every non-pass result and residual fidelity risk.
