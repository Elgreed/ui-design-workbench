...
.agent-handoff-panel{margin:0 0 10px;padding:9px;border:1px solid #cfdcf3;border-radius:9px;background:#f5f8ff}.agent-handoff-panel[hidden]{display:none}.agent-handoff-head{display:flex;align-items:center;gap:7px;color:#2857ac}.agent-handoff-head .icon{width:14px;height:14px}.agent-handoff-status{font-size:9px;font-weight:700}.agent-handoff-meta{margin:5px 0 8px;color:#657386;font-size:8px;line-height:1.4}.agent-handoff-actions{display:grid;grid-template-columns:1fr 1fr;gap:5px}.agent-handoff-action{min-height:28px;border:1px solid #ccd7e7;border-radius:6px;background:#fff;color:#46566c;font-size:7px;cursor:pointer}.review-launcher-action.icon-only{min-height:29px}.review-launcher-actions.with-handoff{grid-template-columns:1fr 34px}.review-launcher-actions.with-handoff .run-review{grid-column:1/-1}
.version-select,.compare-version-select,.compare-version-arrow{display:none!important}.screen-card{content-visibility:visible;contain-intrinsic-size:auto}
.canvas-layer-controls{position:absolute;left:50%;top:54px;z-index:19;display:flex;align-items:center;gap:2px;transform:translateX(-50%);padding:3px;border:1px solid #d8dce2;border-radius:9px;background:rgba(255,255,255,.97);box-shadow:0 5px 18px rgba(26,32,44,.12);backdrop-filter:blur(10px)}.canvas-layer-toggle,.compare-button{height:28px;display:inline-flex;align-items:center;justify-content:center;gap:5px;border:0;border-radius:6px;background:transparent;padding:0 8px;color:#657080;font-size:8px;font-weight:650;cursor:pointer;white-space:nowrap}.canvas-layer-toggle:hover:not(:disabled),.compare-button:hover{background:#f0f2f5;color:#26303d}.canvas-layer-toggle[aria-pressed=true],.compare-button[aria-pressed=true]{background:#273142;color:#fff}.canvas-layer-toggle:disabled{cursor:not-allowed;opacity:.4}.canvas-layer-toggle .icon{width:13px;height:13px}.canvas-layer-count{min-width:14px;padding:1px 4px;border-radius:999px;background:#e6e9ee;color:#596474;font-size:7px;font-variant-numeric:tabular-nums}.canvas-layer-toggle[aria-pressed=true] .canvas-layer-count{background:rgba(255,255,255,.18);color:#fff}.canvas-layer-divider{width:1px;height:18px;margin:0 2px;background:#e1e4e8}.compare-layout-controls{display:flex;gap:2px}.compare-layout-controls[hidden]{display:none}
.finding-canvas-layer{position:absolute;inset:0;z-index:16;pointer-events:none}.finding-pin,.finding-pin-card{position:absolute;pointer-events:auto}.finding-pin{width:24px;height:24px;display:grid;place-items:center;transform:translate(-50%,-50%);border:2px solid #fff;border-radius:999px;background:#dc2626;color:#fff;box-shadow:0 2px 8px rgba(15,23,42,.35);font-size:9px;font-weight:800;cursor:pointer;font-variant-numeric:tabular-nums}.finding-pin[data-severity=blocker]{background:#7f1d1d}.finding-pin[data-severity=medium]{background:#d97706}.finding-pin[data-severity=low]{background:#2563eb}.finding-pin:hover,.finding-pin:focus-visible{transform:translate(-50%,-50%) scale(1.12);outline:3px solid rgba(147,197,253,.8);outline-offset:2px}.finding-pin-card{width:min(280px,calc(100vw - 48px));overflow:hidden;border:1px solid #d7dce3;border-radius:10px;background:#fff;box-shadow:0 14px 38px rgba(20,27,38,.24);color:#303947}.finding-pin-card-header{display:grid;grid-template-columns:24px minmax(0,1fr) 26px;align-items:start;gap:7px;padding:9px 8px 7px}.finding-pin-number{width:22px;height:22px;display:grid;place-items:center;border-radius:999px;background:#dc2626;color:#fff;font-size:8px;font-weight:800}.finding-pin-card-title{margin:1px 0 0;font-size:10px;line-height:1.35}.finding-pin-collapse{width:26px;height:26px;display:grid;place-items:center;border:0;border-radius:6px;background:#f0f2f5;color:#5c6776;cursor:pointer}.finding-pin-collapse:hover{background:#e4e7eb;color:#202936}.finding-pin-collapse .icon{width:12px;height:12px}.finding-pin-card-body{padding:0 10px 10px}.finding-pin-card-meta{margin:0 0 6px;color:#788392;font-size:7px;text-transform:uppercase}.finding-pin-card-body p{margin:5px 0;color:#4f5c6c;font-size:9px;line-height:1.45}.finding-pin-card-body b{color:#2e3846}.finding-pin-card-actions{display:flex;gap:5px;margin-top:8px}.finding-pin-card-actions button{min-height:27px;border:1px solid #d5dbe3;border-radius:6px;background:#fff;padding:4px 7px;color:#465466;font-size:8px;cursor:pointer}.finding-pin-card-actions button.primary{border-color:#315fbd;background:#315fbd;color:#fff}.finding-card-header{display:grid;grid-template-columns:22px minmax(0,1fr) auto}.finding-list-index{width:20px;height:20px;display:grid;place-items:center;border-radius:999px;background:#eef0f3;color:#596474;font-size:7px;font-weight:800;font-variant-numeric:tabular-nums}
...
@container(max-width:760px){.screen-context{display:none}.menu-strip{display:none}.mode-status{display:none}.canvas-tools .button-label{display:none}.canvas-tools .mode-button,.canvas-tools .interaction-button{width:30px;padding:0}.canvas-tools{gap:3px}.canvas-layer-toggle{padding:0 7px}.canvas-layer-toggle .layer-label{display:none}}
...
const versionById=Object.fromEntries(versions.map(v=>[v.id,v]));const baselineVersion=review.baselineVersion&&versionById[review.baselineVersion]?review.baselineVersion:versions[0].id;const initialVersion=review.activeVersion&&versionById[review.activeVersion]?review.activeVersion:versions.at(-1).id;const proposalVersions=()=>versions.filter(version=>version.kind==='proposal');
...
const workbenchSchema=5;const reviewRevision=`${review.revision||'auto'}-${revisionHash(JSON.stringify({workbenchSchema,screens,nodes,versions,audit:expertAudit}))}`;const storageKey=`ui-design-workbench:${ir.project?.name||'project'}:${review.sessionId||'default'}`,legacyStorageKey=`ui-code-preview:${ir.project?.name||'project'}:${review.sessionId||'default'}`,compatibleStorageKeys=[storageKey,legacyStorageKey,...(Array.isArray(review.legacyStorageKeys)?review.legacyStorageKeys:[])];let saved={};try{saved=JSON.parse(compatibleStorageKeys.map(key=>localStorage.getItem(key)).find(Boolean)||'{}')}catch(_){saved={}}const staleReviewSnapshot=saved.revision&&saved.revision!==reviewRevision?saved:null;if(staleReviewSnapshot)saved={};
...
function orderedFindings(values=[...findings]){const sourceOrder=new Map([...findings].map((item,index)=>[item.id,index]));return [...values].sort((a,b)=>(findingRank[b.severity]||0)-(findingRank[a.severity]||0)||(sourceOrder.get(a.id)||0)-(sourceOrder.get(b.id)||0))}
function findingNumber(id){return orderedFindings().findIndex(item=>item.id===id)+1}
function findingsForCanvas(){let result=state.findingFocus.length?findings.filter(item=>state.findingFocus.includes(item.id)):[...findings];if(!state.findingFocus.length&&state.findingFilter==='accepted')result=result.filter(item=>state.findingDecisions[item.id]==='accepted');else if(!state.findingFocus.length&&!['all','linked'].includes(state.findingFilter))result=result.filter(item=>item.severity===state.findingFilter);result=result.filter(item=>{const origin=findingOrigin(item),sourceMatch=state.findingSource==='all'||state.findingSource===origin||(state.findingSource==='systemic'&&item.systemic),screenMatch=state.findingScreen==='all'||item.screenId===state.findingScreen;return sourceMatch&&screenMatch});return orderedFindings(result)}
function activeAfterVersion(){const candidates=proposalVersions();if(!candidates.length)return null;if(versionById[state.compareTargetVersion]?.kind==='proposal')return state.compareTargetVersion;if(versionById[state.activeVersion]?.kind==='proposal')return state.activeVersion;return candidates.at(-1).id}
function syncCanvasLayerControls(){const afterId=activeAfterVersion();if(!afterId){state.showBefore=true;state.showAfter=false;state.activeVersion=baselineVersion}document.querySelectorAll('[data-canvas-layer]').forEach(button=>{const layer=button.dataset.canvasLayer,pressed=layer==='findings'?state.showFindings:layer==='before'?state.showBefore:state.showAfter;button.setAttribute('aria-pressed',String(pressed));if(layer==='after')button.disabled=!afterId});const count=$('.canvas-layer-count');if(count)count.textContent=String(findingsForCanvas().length);const comparing=Boolean(afterId&&state.showBefore&&state.showAfter),layout=$('.compare-layout-controls'),divider=$('.compare-layout-divider');if(layout)layout.hidden=!comparing;if(divider)divider.hidden=!comparing;document.querySelectorAll('[data-compare-mode]').forEach(button=>button.setAttribute('aria-pressed',String(comparing&&button.dataset.compareMode===state.compareMode)))}
function toggleCanvasLayer(layer){if(layer==='findings'){state.showFindings=!state.showFindings;if(!state.showFindings)state.openFindingId=null;persist();syncCanvasLayerControls();renderFindingMarkers();return}const afterId=activeAfterVersion();if(layer==='after'&&!afterId)return;if(layer==='before')state.showBefore=!state.showBefore;if(layer==='after')state.showAfter=!state.showAfter;if(!state.showBefore&&!state.showAfter){if(layer==='before')state.showAfter=true;else state.showBefore=true;showToast('На холсте должна остаться хотя бы одна версия')}if(state.showBefore&&state.showAfter&&afterId){if(state.view!=='compare')state.layerReturnView=state.view;state.compareBaseVersion=baselineVersion;state.compareTargetVersion=afterId;state.activeVersion=afterId;state.view='compare'}else{state.activeVersion=state.showBefore?baselineVersion:afterId;if(state.view==='compare')state.view=state.layerReturnView||'single'}if(versionSelect)versionSelect.value=state.activeVersion;persist();writeLocation(false);renderView()}
function findingPinCard(item,number,x,y,stage){const width=280,height=230,viewportLeft=stage.scrollLeft+10,viewportTop=stage.scrollTop+10,viewportRight=stage.scrollLeft+stage.clientWidth-10,viewportBottom=stage.scrollTop+stage.clientHeight-10;let left=x+15,top=y-14;if(left+width>viewportRight)left=Math.max(viewportLeft,x-width-15);top=Math.max(viewportTop,Math.min(top,viewportBottom-height));return `<article class="finding-pin-card" data-finding-popover="${esc(item.id)}" style="left:${Math.round(left)}px;top:${Math.round(top)}px"><header class="finding-pin-card-header"><span class="finding-pin-number">${number}</span><h3 class="finding-pin-card-title">${esc(item.title)}</h3><button type="button" class="finding-pin-collapse" data-collapse-finding="${esc(item.id)}" aria-label="Свернуть описание проблемы" title="Свернуть в маркер">${iconMarkup('minus')}</button></header><div class="finding-pin-card-body"><div class="finding-pin-card-meta">${esc(item.severity)} · ${esc(item.category||'UI/UX')}</div><p>${esc(item.observation)}</p><p><b>Исправить:</b> ${esc(item.recommendation)}</p><div class="finding-pin-card-actions"><button type="button" class="primary" data-open-finding-list="${esc(item.id)}">В списке проблем</button>${item.proposalVersionId?`<button type="button" data-pin-compare="${esc(item.id)}">Сравнить</button>`:''}</div></div></article>`}
function openFindingInList(id){state.findingFocus=[id];state.findingFilter='linked';setInspectorTab('review');state.reviewSection='problems';persist();renderReviewSections();renderFindings();requestAnimationFrame(()=>document.querySelector(`.finding-card[data-finding-id="${CSS.escape(id)}"]`)?.scrollIntoView({block:'center',behavior:'smooth'}))}
function renderFindingMarkers(){const stage=$('.stage');if(!stage)return;stage.querySelector('.finding-canvas-layer')?.remove();syncCanvasLayerControls();const visible=findingsForCanvas();if(!state.showFindings||!visible.length)return;const layer=document.createElement('div');layer.className='finding-canvas-layer';layer.style.width=`${Math.max(stage.clientWidth,stage.scrollWidth)}px`;layer.style.height=`${Math.max(stage.clientHeight,stage.scrollHeight)}px`;stage.append(layer);const stageRect=stage.getBoundingClientRect(),slots=new Map();let opened=false;for(const item of visible){const devices=[...stage.querySelectorAll(`.device[data-screen-id="${CSS.escape(item.screenId||'')}"]`)];for(const device of devices){const anchor=item.nodeId?device.querySelector(`[data-node-id="${CSS.escape(item.nodeId)}"]`):device.querySelector('.device-content');if(!anchor)continue;const rect=anchor.getBoundingClientRect(),key=`${item.screenId}:${item.nodeId||'screen'}:${device.dataset.versionId}`,slot=slots.get(key)||0;slots.set(key,slot+1);const x=rect.right-stageRect.left+stage.scrollLeft+slot*11,y=rect.top-stageRect.top+stage.scrollTop+Math.min(12,rect.height/2)+slot*7,number=findingNumber(item.id);if(state.openFindingId===item.id&&!opened){layer.insertAdjacentHTML('beforeend',findingPinCard(item,number,x,y,stage));opened=true}else{layer.insertAdjacentHTML('beforeend',`<button type="button" class="finding-pin" data-severity="${esc(item.severity||'medium')}" data-open-finding="${esc(item.id)}" style="left:${Math.round(x)}px;top:${Math.round(y)}px" aria-label="Проблема ${number}: ${esc(item.title)}" title="${esc(item.title)}">${number}</button>`)}}}layer.querySelectorAll('[data-open-finding]').forEach(button=>button.addEventListener('click',event=>{event.stopPropagation();state.openFindingId=button.dataset.openFinding;persist();renderFindingMarkers()}));layer.querySelectorAll('[data-collapse-finding]').forEach(button=>button.addEventListener('click',event=>{event.stopPropagation();state.openFindingId=null;persist();renderFindingMarkers()}));layer.querySelectorAll('[data-open-finding-list]').forEach(button=>button.addEventListener('click',()=>openFindingInList(button.dataset.openFindingList)));layer.querySelectorAll('[data-pin-compare]').forEach(button=>button.addEventListener('click',()=>focusFinding(button.dataset.pinCompare,true)))}
...
function agentTaskContext(kind){const selected=selectedFindings(),openAnnotations=state.annotations.filter(item=>!['resolved','rejected'].includes(item.status||'new')).slice(0,30).map(item=>({id:item.id,screenId:item.screenId,nodeId:item.nodeId,versionId:item.versionId,text:item.text,priority:item.priority,status:item.status}));return {type:'ui-design-workbench-agent-job',version:1,provider:String(previewContext.handoffProvider||'generic'),kind,reviewRevision,project:ir.project?.name||'Project',artifactDir:previewContext.artifactDir||'',scope:state.reviewScope,screenIds:reviewScopeScreens().map(item=>item.id),activeVersion:state.activeVersion,baselineVersion,acceptedFindingIds:selected.map(item=>item.id),annotations:openAnnotations,variantCount:2,allowedWrites:['ui-ir.json','ui-preview.html','*.report.json'],sourceChangeAllowed:false}}
function agentTaskDescriptor(kind='expert'){const artifactDir=String(previewContext.artifactDir||'').trim(),context=agentTaskContext(kind),provider=String(previewContext.handoffProvider||'generic').toLowerCase();if(!artifactDir)return {supported:false,kind,error:'Каталог макета не записан в HTML'};if(kind==='proposal'&&!context.acceptedFindingIds.length)return {supported:false,kind,error:'Сначала выберите проблемы для исправления'};const task=kind==='proposal'?'Создай два осмысленно различающихся варианта исправления только для выбранных проблем.':'Проведи глубокое UI/UX-ревью собранных макетов и создай исправленные Before/After варианты для обоснованных проблем.';const prompt=`Активируй навык ui-design-workbench или прочитай его SKILL.md. В рабочем каталоге находятся ui-ir.json и ui-preview.html. ${task}\n\nКонтекст задания:\n${JSON.stringify(context)}\n\nОбязательный результат:\n1. Прочитай ui-ir.json и повтори нужные детерминированные проверки, чтобы восстановить runtime-проблемы по стабильным ID.\n2. Не изменяй исходный проект и immutable baseline. Работай только с файлами макета в текущем каталоге.\n3. Добавь экспертные findings и sparse proposal-версии в ui-ir.json; сохрани устойчивые ID и привязки к экранам/узлам.\n4. Для выбранных проблем создай до двух вариантов только при реальном UX-компромиссе; иначе один сдержанный вариант.\n5. Пересобери ui-preview.html, выполни strict coverage/platform validation и headless smoke.\n6. В финале перечисли созданные версии и попроси вернуться в макет и нажать «Обновить макет».`;const params=new URLSearchParams({prompt,path:artifactDir}),url=provider==='codex'?`codex://new?${params.toString()}`:null;return {supported:true,kind,path:artifactDir,prompt,url,provider,context}}
async function prepareAgentTask(kind){const descriptor=agentTaskDescriptor(kind);if(!descriptor.supported){showToast(descriptor.error);if(kind==='proposal')downloadJson('ui-fix-request.json',fixRequestPayload());else if(state.diagnostics)downloadJson('ui-expert-review-request.json',expertReviewRequestPayload());return}state.agentHandoff={kind,provider:descriptor.provider,status:'prepared',createdAt:new Date().toISOString(),acceptedFindingIds:descriptor.context.acceptedFindingIds,screenIds:descriptor.context.screenIds,artifactDir:descriptor.path};persist();renderAgentHandoff();renderReviewNextAction();if(descriptor.url){showToast('Задача подготовлена — отправьте запрос в агенте');location.href=descriptor.url;return}try{await navigator.clipboard.writeText(descriptor.prompt);showToast('Запрос для AI-агента скопирован')}catch(_){downloadJson('ui-agent-job.json',{...descriptor.context,prompt:descriptor.prompt});showToast('Задание для AI-агента скачано')}}
async function copyAgentTask(){const kind=state.agentHandoff?.kind||'expert',descriptor=agentTaskDescriptor(kind);if(!descriptor.supported){showToast(descriptor.error);return}try{await navigator.clipboard.writeText(descriptor.prompt);showToast('Запрос для AI-агента скопирован')}catch(_){showToast('Не удалось скопировать запрос')}}
function refreshPreview(){location.reload()}
function renderAgentHandoff(){const panel=$('.agent-handoff-panel'),status=$('.agent-handoff-status'),meta=$('.agent-handoff-meta');if(!panel||!status||!meta)return;const value=state.agentHandoff;panel.hidden=!value;if(!value)return;status.textContent=value.kind==='proposal'?'Задание на вариант подготовлено':'Задание на AI-ревью подготовлено';meta.textContent=`${new Date(value.createdAt).toLocaleString()} · выполните подготовленный запрос в выбранном AI-агенте. После завершения вернитесь сюда и обновите макет.`}
...
$('.run-review')?.addEventListener('click',runAutomatedReview);document.querySelectorAll('.open-agent-review').forEach(button=>button.addEventListener('click',()=>{if(state.diagnostics)prepareAgentTask('expert')}));document.querySelectorAll('.export-review-request').forEach(button=>button.addEventListener('click',()=>{if(!state.diagnostics)return;downloadJson('ui-expert-review-request.json',expertReviewRequestPayload());showToast('JSON-задание для AI-ревью скачано')}));
$('.export-fix-request')?.addEventListener('click',()=>{if(!selectedFindings().length)return;downloadJson('ui-fix-request.json',fixRequestPayload());showToast('Задание для AI-агента подготовлено')});$('.copy-fix-request')?.addEventListener('click',async()=>{if(!selectedFindings().length)return;try{await navigator.clipboard.writeText(JSON.stringify(fixRequestPayload(),null,2));showToast('Задание для AI-агента скопировано')}catch(_){showToast('Не удалось скопировать — используйте загрузку файла')}});
...
def render_html(
    ...
    encoded_context = json.dumps(
        {
            ...
            "handoffProvider": handoff_provider,
        },
        ensure_ascii=False,
        separators=(",", ":"),
    ).replace("<", "\\u003c")
    ...
    title = f'{project.get("name", "Project")} · UI Design Workbench'
...
<div class="review-app">
  ...
  <main class="workspace">
    <header class="workbench-header">
      <div class="menu-bar">
        <div class="app-mark">{icon("logo", "app-logo")}<span>UI Design Workbench</span><span class="revision-badge" title="Ревизия макета"></span></div>
        ...
        <select class="version-select" aria-label="Активная версия макета" hidden></select>
    ...
    <section class="stage" aria-label="UI Design Workbench canvas"></section>
    ...
    <div class="canvas-layer-controls" aria-label="Видимость элементов на макете">
      <button type="button" class="canvas-layer-toggle" data-canvas-layer="findings" aria-pressed="true" aria-label="Показывать проблемы на макете" title="Показать или скрыть проблемы">{icon("review")}<span class="layer-label">Проблемы</span><span class="canvas-layer-count">0</span></button>
      <span class="canvas-layer-divider"></span>
      <button type="button" class="canvas-layer-toggle" data-canvas-layer="before" aria-pressed="false" title="Показать исходную версию">Before</button>
      <button type="button" class="canvas-layer-toggle" data-canvas-layer="after" aria-pressed="true" title="Показать предложенную версию">After</button>
      <span class="canvas-layer-divider compare-layout-divider"></span>
      <div class="compare-layout-controls" aria-label="Способ сравнения"><button type="button" class="compare-button" data-compare-mode="split">Рядом</button><button type="button" class="compare-button" data-compare-mode="overlay">Наложение</button></div>
    </div>
  ...
  <aside class="inspector">
    ...
    <section class="inspector-pane" data-inspector-pane="review">
      ...
      <section class="review-workspace" data-review-workspace="changes" aria-label="Изменения" hidden>
        ...
        <section class="agent-handoff-panel" hidden><div class="agent-handoff-head">{icon("external")}<span class="agent-handoff-status"></span></div><p class="agent-handoff-meta"></p><div class="agent-handoff-actions"><button type="button" class="agent-handoff-action copy-agent-task">Скопировать запрос</button><button type="button" class="agent-handoff-action refresh-preview">Обновить макет</button></div></section>
        <section class="import-review-panel"><div class="import-review-head"><h3>Результат AI-агента</h3><span class="import-review-badge">не импортирован</span></div><p class="import-review-copy">Агент может пересобрать файлы макета напрямую. Импорт JSON остаётся переносимым резервным способом.</p><div class="import-review-actions"><button type="button" class="import-review-action import-review-result">Импортировать JSON</button><button type="button" class="import-review-action export-review-request" disabled>Скачать задание</button></div><input type="file" class="import-review-file" accept="application/json,.json" hidden><div class="import-review-status" role="status"></div></section>
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="generic",
        help="Optional handoff adapter. Generic copies/downloads a portable job; codex also enables codex://new.",
    )
...
def main() -> int:
    ...
    output.write_text(render_html(rendered_ir, output.parent, args.agent), encoding="utf-8")
    print(str(output))
    return 0
