...
Set `review.revision` to a stable human-readable revision for the review bundle. The renderer combines it with a target-content hash that covers screens, nodes, themes, versions, and audit data but excludes the renderer/workbench schema. Store the workbench and diagnostics-engine versions separately. Re-rendering or upgrading the HTML shell preserves review runs, selections, decisions, annotations, and view context when target content is unchanged. It preserves runtime diagnostics only when their engine version matches; otherwise it removes technical findings and requires a fresh run without discarding human review state. A legacy snapshot from the same `review.revision` lineage is migrated automatically once and stamped with the separate target revision. When target content really changes, do not silently replay stale references: keep the old snapshot intact until the reviewer migrates compatible state or discards it. Never migrate runtime findings across target revisions; recompute them.
...
The primary `Review` entry point is one button, `Запустить ревью`. It runs the configured scenarios across every assembled screen and fixed profile, replaces the previous runtime-generated set with evidence-backed cards for the current non-pass results, removes cards whose checks now pass, focuses that generated set in the findings list, and leaves the technical report under the collapsed `Диагностика` section. A repeated run reuses deterministic diagnostic finding IDs instead of duplicating cards. The launcher must show scope, running state, final issue count, and a clear rerun label.

The Review panel uses three isolated workspaces: `Проверка`, `Проблемы`, and `Исправление`. Проверка reports exact screen/profile/check/state coverage and named run history. Проблемы groups automated failures by causal scenario and screen, keeps systemic findings ahead of instances, and supports source/screen/severity filtering plus bulk decisions. Исправление shows one concise current-state summary, result import, version comparison, design approval, and the source-planning boundary; it must not repeat another step diagram. A single sticky primary action advances to the next valid stage.
...
For an explicit review headless/local pass, open the generated file with `?diagnostics=run&diagnostic-mode=review`; the runner starts automatically and exposes the same report in the diagnostics panel. This remains an offline HTML check and does not execute the application. Ordinary reconstruction uses `diagnostic-mode=projection` and never creates review findings.

The dependency-free Node helper performs that pass and writes a reusable report:

```powershell
node <skill-dir>/scripts/smoke_preview.js <review-dir>/ui-preview.html --mode review --output <review-dir>/ui-diagnostics.json
```
...
Projection scenarios check screen/node IR-to-DOM transfer, source-evidenced variant rendering, navigation-target references, zoom-label/reset synchronization, stable overview-card geometry and scroll bounds, menu exclusivity, and the workbench's theme/state controls. They do not inspect product contrast, keyboard access, target size, typography quality, spacing, overlap, overflow, or user-flow reachability and cannot create findings. Review mode adds those UI/UX checks. Navigation reachability is assessed only when the IR declares an entry screen through `scenario.entryScreenIds` or `screen.navigationEntry`; otherwise review diagnostics validate target references without inventing a start node. Group repeated review non-pass checks by scenario and screen into one systemic runtime finding with measured instances. Exported review findings merge into `review.audit.findings`, so they remain actionable in the next revision.
...
Each finding has one stable review number used in both the Problems list and its canvas marker. A reliably mapped screen/node receives a numbered circle; clicking it opens one anchored description card and the top-right collapse action restores the circle. Current finding filters and the Problems visibility toggle update list and canvas together. Each finding can focus its affected screen/node and, when linked to `proposalVersionId`, open the dedicated Compare view. A proposal declares visually addressed corrections in `resolvedFindingIds`; this is proposal coverage, not proof that project files changed. Baseline markers render only on the immutable reviewed version and on the original side of Compare. The reviewer chooses `Выбрать для применения`, `Не исправлять`, or `Позже`. The correction queue reports how many selected findings already have a proposal, how many need a new one, and how many were applied and verified. `Создать вариант макета` prepares an agent task scoped to the review directory. The task includes selected finding IDs, active and baseline versions, annotations, review scope, and `sourceChangeAllowed: false`; it may create up to two alternatives only when a real UX tradeoff exists. `Скопировать задание` and `ui-fix-request.json` remain portable fallbacks.

The queue communicates six concrete outcomes: `Найдено → Выбрано → Учтено в новом макете → Подтверждено → Применено → Проверено`. The safe path previews corrections first and exposes `Применить исправления` only on step 3, after every selected preview-solvable finding is addressed in the active proposal and the user approves it. Approval still leaves the persistent status `в проект не применён`. A separate advanced source action is the explicit fast-path authorization: it prepares one implementation job for every current finding and bypasses proposal approval. Implementation jobs set `sourceChangeAllowed: true`, include `projectRoot`, stable finding IDs, annotations, and source targets, and must change real project files rather than merely rebuilding the proposal.

After implementation, run incremental `uidw sync` and only targeted checks for affected screens, finding IDs, and project modules. Record a finding-level `verification` only for passing targeted checks; legacy `status: resolved`, a prepared handoff, proposal coverage, or proposal approval is not completion. Do not automatically repeat the full AI review; that is an explicit user action. The agent's final chat response must be a plain numbered list of every selected finding with screen, problem, implemented fix, changed files, verification result, and any remaining blocker. Keep this textual report in chat, not as a duplicate HTML block.
...
1. Explicitly save or copy `ui-review-state.json` from the preview. The File menu also restores this file directly; saving is never automatic.
...
```powershell
python <skill-dir>/scripts/merge_review_state.py <review-dir>/ui-ir.json <review-dir>/ui-review-state.json --output <review-dir>/ui-ir-reviewed.json
```
...
`merge_review_state.py` accepts `ui-review-state.json`, the legacy `ui-review-feedback.json`, or the wrapped `ui-fix-request.json`. For a fix request it also preserves the selected IDs and requested action as `review.correctionRequest` before a new proposal is authored.
