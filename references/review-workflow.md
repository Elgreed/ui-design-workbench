...
Set `review.revision` to a stable human-readable revision for the review bundle. The renderer combines it with a target-content hash that covers screens, nodes, themes, versions, and audit data but excludes the renderer/workbench schema. Store the workbench schema separately. Re-rendering or upgrading the HTML shell must preserve diagnostics, review runs, selections, decisions, annotations, and view context when the target-content revision is unchanged. A legacy snapshot from the same `review.revision` lineage is migrated automatically once and stamped with the separate target revision. When target content really changes, do not silently replay stale references: keep the old snapshot intact until the reviewer migrates compatible state or discards it. Migration must filter missing screen, node, finding, and version references.
